...
export function parseBlogPost(html, sourcePath) {
  ...
  if (!heroImage || heroImage.includes('logo')) {
    heroImage = '/image-watermarked/project-item/project-item-12.jpg';
  }
  ...
  // Strip leftover gallery section markup, containers, overlays and captions from articleHtml
  articleHtml = articleHtml
    .replace(/<section\b[^>]*id=["']gallery["'][^>]*>[\s\S]*?<\/section>/gi, '')
    .replace(/<div\b[^>]*class=["'][^"']*gallery-grid-container[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, '')
    .replace(/<div\b[^>]*class=["'][^"']*gallery-grid[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, '')
    .replace(/<div\b[^>]*class=["'][^"']*gallery-item[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, '')
    .replace(/<div\b[^>]*class=["'][^"']*gallery-overlay[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, '')
    .replace(/<span\b[^>]*class=["'][^"']*gallery-caption[^"']*["'][^>]*>[\s\S]*?<\/span>/gi, '')
    .replace(/<span\b[^>]*class=["'][^"']*gallery-tag[^"']*["'][^>]*>[\s\S]*?<\/span>/gi, '');
